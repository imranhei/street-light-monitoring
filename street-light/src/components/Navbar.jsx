  ...
  return (
    <div className='flex justify-between items-center px-10 py-2 bg-indigo-950 text-white fixed top-0 left-0 w-full z-20'>
        ...
        <div className={`flex flex-col md:flex-row gap-x-10 gap-y-4 text-sm md:static absolute bg-indigo-950 px-10 md:px-0 py-4 md:py-0 transition-all duration-500 ease-in top-12 ${open ? 'right-0':'right-[-200px]'}`}>
            <Link to='/home' className={`hover:text-rose-400 cursor-pointer w-fit ${location.pathname === '/' ? 'border-b' : ''}`}>Dashboard</Link>
            <Link to='/inventory' className={`hover:text-rose-400 cursor-pointer w-fit ${location.pathname === '/inventory' ? 'border-b' : ''}`}>Inventory</Link>
            <Link to='/view' className={`hover:text-rose-400 cursor-pointer w-fit ${location.pathname === '/view' ? 'border-b' : ''}`}>View</Link>
            <Link to='/alarm' className={`hover:text-rose-400 cursor-pointer w-fit ${location.pathname === '/alarm' ? 'border-b' : ''}`}>Alarm</Link>
            <Link to='/profile' className={`hover:text-rose-400 cursor-pointer w-fit ${location.pathname === '/admin' ? 'border-b' : ''}`}>Admin</Link>
