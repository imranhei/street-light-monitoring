...
export default function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    if (validateForm()) {
      if(email === loginData.email && pass === loginData.pass){
        dispatch(logIn());
        navigate('/home');
      }
      else{
  ...
  const validateForm = () => {
    ...
    if (!pass) {
      ...
      errors.pass = "Password must have at least 1 lowercase letter, 1 uppercase letter, 1 digit, and a minimum length of 8 characters";
      isValid = false;
    }
    setErrors(errors);
    return isValid;
        ...
        <div className='w-72 h-[450px] bg-white flex flex-col gap-1 items-center justify-center'>
          ...
          <div className='w-60 text-sm'>
            ...
            <div className='border-b-2 pb-1 flex items-center focus-within:border-teal-400'>
              ...
              <input onChange={e => setEmail(e.target.value)} className='outline-none w-full pl-1' type='text' placeholder='Type your email : imran@gmail.com'/>
            </div>
            ...
            <div className='border-b-2 pb-1 flex items-center focus-within:border-teal-400'>
              ...
              <input onChange={e => setPass(e.target.value)} className='outline-none w-full pl-1' type='password' placeholder='Type your password : Ventia12'/>
            </div>
            <h1 className="text-xs text-red-400">{errors.pass}</h1>
